...
namespace GDBH.DAO
{
    class BH_GioiTinh_DAO
    {

        private static BH_GioiTinh_DAO instance;

        public static BH_GioiTinh_DAO Instance
        {
            get { if (instance == null) instance = new BH_GioiTinh_DAO(); return BH_GioiTinh_DAO.instance; }
            private set { BH_GioiTinh_DAO.instance = value; }
        }
        private BH_GioiTinh_DAO() { }

        public DataTable Load_TheBhyt(string datathang)
        {
            string query = string.Format("SELECT LOI, Thang,TENKP,MABN, NGVAO ,NGRA ,HOTEN, NAMSINH ,PHAI ,GIOITINH_TC MAKQ_TC ,GHICHU_TC,  ID, CHECK_OK  FROM HSOFTDKBD.CDT_CHECKBHYT " +
                "Where (SUBSTR(MATHE,0,2) NOT IN ('QN','CA') AND THANG = '" + datathang + "' AND TENKP != 'Khoa Sơ Sinh' AND CHECK_OK = '0' AND PHAI != GIOITINH_TC)" +                
                "ORDER BY LOI ASC");             
