        ...
        public DataTable Load_Ngaygiuong(string datathang)
        {
            ...
            string query = string.Format("select distinct  LL.ID,  ll.mabn,     BN.HOTEN,KP.TENKP KHOAXV, TO_CHAR(LL.NGAYVAO,'DD_MM_YYYY_HH24:MI') NGAYVAO,TO_CHAR(LL.NGAYRA,'DD_MM_YYYY_HH24:MI') NGAYRA,     (trunc(ll.NGAYRA-ll.NGAYVAO)*24 + to_number(to_char(trunc(sysdate)+(ll.NGAYRA-ll.NGAYVAO),'HH24.MI'))) GIO_PHUT,    ll.ketqua,    ll.tinhtrang,    (trunc(ll.ngayra)- trunc(ll.ngayvao) +1) NGAYDT,    sum(ct.soluong) NGAYGIUONG from " + datathang + ".v_m38ct ct inner join " + datathang + ".v_m38ll ll on ct.id = ll.id INNER JOIN hsoftdkbd.btdkp_bv KP ON KP.MAKP = LL.MAKP INNER JOIN HSOFTDKBD.BTDBN BN ON BN.MABN = LL.MABN where ct.mavp in " + dmgiuong + " AND LL.ID NOT IN(SELECT ID FROM hsoftdkbd.CDT_CHECKNGAYGIUONG) group by ll.mabn, ll.id,BN.HOTEN,KP.TENKP, TO_CHAR(LL.NGAYVAO,'DD_MM_YYYY_HH24:MI'),TO_CHAR(LL.NGAYRA,'DD_MM_YYYY_HH24:MI'), (trunc(ll.NGAYRA-ll.NGAYVAO)*24 + to_number(to_char(trunc(sysdate)+(ll.NGAYRA-ll.NGAYVAO),'HH24.MI'))),ll.ketqua,ll.tinhtrang, (trunc(ll.ngayra)- trunc(ll.ngayvao) +1)" +
                "having ((ll.ketqua = 1 and (ll.tinhtrang = 1 or ll.tinhtrang = 3)) and ( sum(ct.soluong) >= (trunc(ll.ngayra)- trunc(ll.ngayvao) +1)))"+
                    ...
                    "or ((trunc(ll.NGAYRA-ll.NGAYVAO)*24 + to_number(to_char(trunc(sysdate)+(ll.NGAYRA-ll.NGAYVAO),'HH24.MI'))) > 4 and (trunc(ll.NGAYRA-ll.NGAYVAO)*24 + to_number(to_char(trunc(sysdate)+(ll.NGAYRA-ll.NGAYVAO),'HH24.MI'))) < =24 and sum(ct.soluong)>1)" +
                    "or ((trunc(ll.NGAYRA-ll.NGAYVAO)*24 + to_number(to_char(trunc(sysdate)+(ll.NGAYRA-ll.NGAYVAO),'HH24.MI'))) < =4)"+
                    "or ((ll.ketqua = 1 and ll.tinhtrang = 2) OR (ll.ketqua = 1 and ll.tinhtrang = 4) OR (ll.ketqua = 2 and ll.tinhtrang = 4) OR  (ll.ketqua = 4 and ll.tinhtrang = 1) OR (ll.ketqua = 5 and ll.tinhtrang = 1) OR (ll.ketqua = 5 and ll.tinhtrang = 2) OR (ll.ketqua = 5 and ll.tinhtrang = 3))"); 
                
            return DataProvider.Instance.ExecuteQuery(query);
        }
        public bool Insert_Id_Ngaygiuong(string id)
        {
            string query = string.Format("insert into hsoftdkbd.CDT_CHECKNGAYGIUONG (ID) VALUES  ('{0}')", id);
