...
namespace GDBH.DAO
{
    class BH_BaoGiam_DAO
    {

        private static BH_BaoGiam_DAO instance;

        public static BH_BaoGiam_DAO Instance
        {
            get { if (instance == null) instance = new BH_BaoGiam_DAO(); return BH_BaoGiam_DAO.instance; }
            private set { BH_BaoGiam_DAO.instance = value; }
        }
        private BH_BaoGiam_DAO() { }

        public DataTable Load_TheBhyt(string datathang)
        {
            string query = string.Format("SELECT LOI, Thang,TENKP,MABN, NGVAO ,NGRA ,HOTEN, NAMSINH ,PHAI ,MATHE ,  MAKQ_TC ,GHICHU_TC,  ID, CHECK_OK  FROM HSOFTDKBD.CDT_CHECKBHYT " +
                "WHERE (SUBSTR(MATHE,0,2) NOT IN ('QN','CA') AND THANG = '" + datathang + "' AND TENKP != 'Khoa Sơ Sinh' AND CHECK_OK = '0' AND MAKQ_TC in  ( '010', '050', '053','051', '052', '060','061', '070','110', '120', '124','121', '122','123', '205'))" +
                "" +
                "ORDER BY LOI ASC"); 
