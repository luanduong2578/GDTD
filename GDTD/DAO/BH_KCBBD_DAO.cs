...
namespace GDBH.DAO
{
    class BH_KCBBD_DAO
    {
        ...
        private static BH_KCBBD_DAO instance;

        public static BH_KCBBD_DAO Instance
        {
            get { if (instance == null) instance = new BH_KCBBD_DAO(); return BH_KCBBD_DAO.instance; }
            private set { BH_KCBBD_DAO.instance = value; }
        }
        private BH_KCBBD_DAO() { }

        public DataTable Load_TheBhyt(string datathang)
        {
            string query = string.Format("SELECT LOI, Thang,TENKP,MABN, NGVAO ,NGRA ,HOTEN, NAMSINH ,PHAI, KCBBD ,KCBBD_TC ,  MAKQ_TC ,GHICHU_TC,  ID, CHECK_OK  FROM HSOFTDKBD.CDT_CHECKBHYT " +
                "WHERE " +
                "(SUBSTR(MATHE,0,2) NOT IN ('QN','CA') AND THANG = '" + datathang + "' AND TENKP != 'Khoa Sơ Sinh' AND CHECK_OK = '0' AND (KCBBD != KCBBD_TC OR KCBBD IS NULL OR KCBBD_TC IS NULL)) AND MAKQ_TC != '401' " +
                "ORDER BY LOI ASC"); 
